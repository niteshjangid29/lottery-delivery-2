...
const LotteryTicketCard = () => {
  ...
  const filteredTickets = tickets.filter((ticket) => Number(ticket.prize) <= sliderValue);

  return (
    <div className="flex justify-center items-center py-5" >
      <div className="w-full max-w-5xl bg-white border-3 border-yellow-500 rounded-lg px-4">
        {/* Filter Button */}
        <div className="flex justify-end items-center mb-4 relative z-40 opacity-90" ref={filterRef}>
          <button
          ...
          {showSlider && (
            <div className="absolute top-10 right-0 bg-white shadow-lg px-4 py-2 rounded-lg border border-gray-200 w-50" ref={dropdownRef}>
        ...
        {filteredTickets.length > 0 ? (
          Array.from({ length: Math.ceil(filteredTickets.length / 3) }).map((_, rowIndex) => (
          ...
          <p className="text-gray-500 text-center mt-4">No tickets available under the selected price range.</p>
        )}
      </div>
    </div>
  );
